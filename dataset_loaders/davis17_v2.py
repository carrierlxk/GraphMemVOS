...
class DAVIS17V2(torch.utils.data.Dataset):
    def __init__(self, root_path, version, image_set, image_read=get_default_image_read(),
                 anno_read=get_default_anno_read(), image_label_read=None,
...
#        assert self._version == '2017', "Only the 2017 version is supported for training as of now"
        ...
        if self.transform is not None:
            ...
            for idx in frame_ids:
                ...
                my_gt = np.array(Image.open(self._full_anno_path(seqname, idx)))
                 #print('davis range:', np.max(my_image), np.min(my_image), np.max(my_gt), np.min(my_gt))
        ...
        if self._joint_transform is not None:
            ...
            #                      for idx in frame_ids])
            #print('davis range:', torch.max(images), torch.min(images))
            #segannos = torch.stack([self._anno_read(self._full_anno_path(seqname, idx))
            #                      for idx in frame_ids])
            images = []
            segannos = []
            for idx in frame_ids:
                temp1,temp2 = self._image_anno_read(self._full_image_path(seqname, idx),self._full_anno_path(seqname, idx))
                images.append(temp1)
